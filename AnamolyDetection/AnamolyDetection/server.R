...
server <- function(input, output) {
  ...
  output$Etab3p1<-renderPlot(ggplot(data=data, aes(x=MonthlyCharges, color=Churn))+
                               geom_histogram(aes(y=..density..),fill="White", binwidth =input$slider)+
  ...
  #model
  output$Mlr<-renderPrint(print(LogModel))
  output$MTree<-renderPlot(plot(tree))
  output$Mrf<-renderPrint(print(rfModel))
  output$Mrfp<-renderPlot(tuneRF(Rtraining[, -18], Rtraining[, 18], stepFactor = 0.5, 
  ...
  output$MEtreeAccuracy <- renderValueBox({
   valueBox(
      format(round((sum(diag(tab2))/sum(tab2))*100,2)), "Decision Tree Accuracy", icon = icon("Accuracy", lib = "glyphicon"),
      color = "purple")})
    output$MEtreePrecision<- renderValueBox({
      valueBox(
        format(round(treeprecision*100,2)), "Decision Tree Precision", icon = icon("Accuracy", lib = "glyphicon"),
        color = "blue")
    })
  ...
  output$MErfAccuracy <- renderValueBox({
    valueBox(
      format(round((sum(diag(tab2))/sum(tab2))*100,2)), "Random Forest Accuracy", icon = icon("Accuracy", lib = "glyphicon"),
      color = "purple")})
